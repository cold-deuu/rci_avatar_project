...
if len(connected_devices) == 0:
    print("No inverse3 found")
    exit()
if len(connected_handles) == 0:
...
versegrip_quat = [0, 0, 0, 0]


def handle_thread():
    global run, versegrip_quat
    handle_time = time.perf_counter()
    count = 0
    while run:
        count += 1
        response = versegrip.GetVersegripStatus()
        versegrip_quat = response["quaternion"]
...
def inverse_thread():
    ...
    while run:
        ...
        if time.perf_counter() - inverse_time > 1:
            print(
                "Inverse thread rate: "
                + str(count / (time.perf_counter() - inverse_time))
            )
            inverse_time = time.perf_counter()
            count = 0
...
while True:
    ...
    quat = pyquaternion.Quaternion(
        versegrip_quat[0], versegrip_quat[1], versegrip_quat[2], versegrip_quat[3]
    )
    # rotate the quaternion 180 degrees around the z axis
    # quat = pyquaternion.Quaternion(axis=[0, 0, 1], angle=pi) * quat
